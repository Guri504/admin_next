...
const Membership_Plans_Listing = () => {
    ...
    const updatePlanStatus = async (id, newStatus) => {
        try {
            let resp = await putApi(`admin/membershipPlan/edit/${id}`, {
                status: newStatus
            });
            if (resp.status) {
    ...
    return (
        <div className='right_side'>
            <NavBottom title="Manage Membership Plan Listing">
                <Link href={'/membership_plans/add'} className='back_btn'>NEW</Link>
                                ...
                                <Row>
                                    <Col xxl={8} xl={8} lg={8} md={5} sm={5} xs={12}>
                                        <div className='header_left'>
                                            <div className='heading'>Here Is Your Membership Plan Listing!</div>
                                        </div>
                                    </Col>
                                                ...
                                                <div className='actions_right'>
                                                    <Dropdown>
                                                        ...
                                                        <Dropdown.Menu>
                                                            ...
                                                            <Dropdown.Item onClick={() => softDeleteManyApi('membership_plans', check, listing)}>
                                        ...
                                        <tbody>
                                            {planList?.length > 0 && planList.map((plan, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td><div className='tab'><Link href={`/membership_plans/view/${plan._id}`}>{plan.type}</Link></div></td>
                                                    <td>{plan.price}</td>
                                                    <td>
                                                        <Form.Group className='form-group'>
                                                            <Form.Check
...
export default Membership_Plans_Listing;