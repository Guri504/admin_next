...
const VideosListing = () => {
    ...
    const getListing = async () => {
        try {
            let resp = await getApi('admin/videos');
            if (resp.status) {
    ...
    const statusUpdate = async (id, status) => {
        try {
            let resp = await putApi(`admin/video/edit/${id}`, { status: status });
            if (resp.status) {
    ...
    return (
        <div className='right_side'>
            <NavBottom title="Manage Videos">
                <Link href={'/videos/add'} className='back_btn'>NEW</Link>
                                ...
                                <Row>
                                    <Col xxl={8} xl={8} lg={8} md={5} sm={5} xs={12}>
                                        <div className='header_left'>
                                            <div className='heading'>Here Is Your Videos Listing!</div>
                                        </div>
                                    </Col>
                                                    ...
                                                    return <tr key={i}>
                                                        ...
                                                        <td>
                                                            <Form.Group className='form-group'>
                                                                <Form.Check
                                                                    ...
                                                                    onChange={() => statusUpdate(video?._id, video?.status === 1 ? 0 : 1)}
                                                                />
                                                            </Form.Group>
                                                        </td>
                                                        ...
                                                        <td>
                                                            <Dropdown>
                                                                ...
                                                                <Dropdown.Menu>
                                                                    <Dropdown.Item href={`/videos/edit/${video?._id}`}><span className='edit'>
