...
const Products_Catgeory_Edit = () => {
    ...
    const categorySubmit = async (e) => {
        ...
        try {
            let resp = await putApi(`admin/products-category/edit/${id}`, finalData)
            if (resp.status) {
                toast(resp.message)
                setTimeout(() => router.push('/products_category'), 1500)
            }
        } catch (error) {
            console.log(error)
            toast(resp.message)
    ...
    return (
        <div className='right_side'>
            <NavBottom backUrl="/products_category" />
            <div className="right_area top_spacing">
                <Card>
                    <div className='card-header'>
                        <div className='header_left'>
                            <div className='heading'>
                                Update Your Product Category Here
                            </div>
                        </div>
                    </div>
                            ...
                            <Row>
                                <Col xxl={12} xl={12} lg={12} md={12} sm={12} xs={12}>
                                    <Form.Group className='form-group'>
                                        <Form.Label> Category Title<span>*</span></Form.Label>
                                        <Form.Control
                                            type="text"
                                            placeholder="Enter Category Title"
                                            name='categoryTitle'
                                            defaultValue={defualtData?.categoryTitle}
                                        />
                                    </Form.Group>
                                </Col>
                                <Col xxl={12} xl={12} lg={12} md={12} sm={12} xs={12}>
                                    <Form.Group className='form-group'>
                                        <Form.Label> Category Description <span>*</span></Form.Label>
                                        <CustomEditor
                                            name='description'
...
export default Products_Catgeory_Edit;