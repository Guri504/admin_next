...
const Products_Sizes_Edit = () => {
    ...
    const sizeUpdate = async (e) => {
        e.preventDefault();
        const formdata = new FormData(e.target);
        let finalData = Object.fromEntries(formdata.entries())
        try {
            let resp = await putApi(`admin/product-size/edit/${id}`, finalData)
            if (resp.status) {
                toast(resp.message)
                setTimeout(() => router.push('/products_sizes'), 1500)
            }
        } catch (error) {
            console.log(error)
            toast(resp.message)
    ...
    return (
        <div className='right_side'>
            <NavBottom backUrl="/products_sizes" />
            <div className="right_area top_spacing">
                <Card>
                    <div className='card-header'>
                        <div className='header_left'>
                            <div className='heading'>
                                Update Your Product Size Here
                            </div>
                        </div>
                    </div>
                    <div className='card-body'>
                        <Form onSubmit={sizeUpdate}>
                            <Row>
                                <Col xxl={12} xl={12} lg={12} md={12} sm={12} xs={12}>
                                    <Form.Group className='form-group'>
                                        <Form.Label> Size Title<span>*</span></Form.Label>
                                        <Form.Control
                                            type="text"
                                            placeholder="Enter Category Title"
                                            name='sizeTitle'
                                            defaultValue={defualtData?.sizeTitle}
                                        />
                                    </Form.Group>
                                </Col>
                                <Col xxl={12} xl={12} lg={12} md={12} sm={12} xs={12}>
                                    <Form.Group className='form-group'>
                                        <Form.Label> Size Description <span>*</span></Form.Label>
                                        <CustomEditor
                                            name='description'
...
export default Products_Sizes_Edit;