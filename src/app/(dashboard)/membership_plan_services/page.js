...
const Membership_Plan_Service_Listing = () => {
    ...
    const listing = async () => {
        try {
            let resp = await getApi('admin/membershipPlanServices');
            console.log(resp)
            if (resp.status) {
    ...
    const updateServiceStatus = async (id, newStatus) => {
        try {
            let resp = await putApi(`admin/membershipPlanService/edit/${id}`, {
                status: newStatus
            });
            if (resp.status) {
    ...
    return (
        <div className='right_side'>
            <NavBottom title="Manage Membership Plan Service Listing">
                <Link href={'/membership_plan_services/add'} className='back_btn'>NEW</Link>
                                ...
                                <Row>
                                    <Col xxl={8} xl={8} lg={8} md={5} sm={5} xs={12}>
                                        <div className='header_left'>
                                            <div className='heading'>Here Is Your Membership Plan Service Listing!</div>
                                        </div>
                                    </Col>
                                                ...
                                                <div className='actions_right'>
                                                    <Dropdown>
                                                        ...
                                                        <Dropdown.Menu>
                                                            ...
                                                            <Dropdown.Item onClick={() => softDeleteManyApi('membership_plan_services', check, listing)}>
                                        ...
                                        <tbody>
                                            {serviceList?.length > 0 && serviceList.map((service, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td><div className='tab'><Link href={`/membership_plan_services/view/${service._id}`}>{service.title}</Link></div></td>
                                                    <td>
                                                        <Form.Group className='form-group'>
                                                            <Form.Check
                                                                type="switch"
                                                    ...
                                                    <td>
                                                        <Dropdown>
                                                            ...
                                                            <Dropdown.Menu>
                                                                <Dropdown.Item href={`/membership_plan_services/edit/${service._id}`}><span className='edit'>
                                                                    <FontAwesomeIcon icon={faEdit} /></span> Edit
                                                                </Dropdown.Item>
                                                                <Dropdown.Item href={`/membership_plan_services/view/${service._id}`}>
...
export default Membership_Plan_Service_Listing;